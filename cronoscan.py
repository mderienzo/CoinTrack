...
def get_transactions(address):
    ...
    transactions_url = make_api_url(module = "account",  action = "txlist", address = address, startblock=0, endblock=99999999, page=1, offset=10000, sort="asc")
    response = get(transactions_url)
    data = response.json()["result"]

    internal_tx_url = make_api_url(module = "account", action =  "txlistinternal",address =  address, startblock=0, endblock=99999999, page=1, offset=10000, sort="asc")
    response2 = get(internal_tx_url)
    data2 = response2.json()["result"]

    transactions_url = make_api_url(module = "account", action =  "tokentx", address = address, startblock=0, endblock=99999999, page=1, offset=10000, sort="asc")
    response = get(transactions_url)
...
def get_clean_dataframe(address):
    ...
    for tx in data:
        ...
        if "gasPrice" in tx:
            gas = int(tx["gasUsed"]) * int(tx["gasPrice"]) / decimal
        else:
            gas = int(tx["gasUsed"]) / decimal
...
def composizione_cronos(address, anni_da_dichiarare):
...
if __name__ == "__main__":
    ...
    dict_rep_fin_per_moneta = composizione_cronos(address, [2020,2021,2022,2023,2024])
